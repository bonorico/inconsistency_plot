...
inconsistency_graph <- function(netmeta_obj, eff_type = "fixed", alpha = 0.05, color_by = "significance", include_labels = TRUE){
  ...
  graph_df <- left_join(estimates, diff, by = "comparison") %>%
    ...
    # we can most likely drop name = diff here
              mutate(significance = if_else(diff.pval <= alpha, "Significant", "Non-significant"),
                     # here to plot vertically do x_coord = direct.estimate; y_coord = value + direct.estimate
                     x_coord = value + indirect.estimate,
                     y_coord = indirect.estimate)

  # Find the max coordinate magnitude 
  max_coord <- ceiling(max(abs(graph_df$x_coord), abs(graph_df$y_coord)))
  ...
  if (include_labels == T){
    ...
    text_geom <- geom_text_repel(
            data = graph_df %>% filter(name == "diff.up"),
                aes(x = x_coord,
                    y = y_coord,
                    label = comparison,
